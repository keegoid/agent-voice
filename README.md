...
play generated WAVs longer than 180 seconds by default, then bounds `afplay` with
a timeout capped by that same limit. Use `AGENT_VOICE_MAX_PLAYBACK_SECONDS` or
`--max-playback-seconds` to change the refusal cap, and
`AGENT_VOICE_PLAYBACK_TIMEOUT_SECONDS` or `--play-timeout` to override only the
`afplay` wait once playback starts.
